...
function CancunProperties() {
  ...
  useEffect(() => {
    const CancunProperties = allProperties.filter(
      p => p.ubicacion === 'Cancún' 
    );


    setFilteredProperties(CancunProperties);
  }, [allProperties]);
  useEffect(() => {
  ...
  return (
    <>
      <div className="mt-24 mx-auto justify-center">
        ...
        {language === es ? es.propertiesIn : en.propertiesIn} Cancún
        </h2>
