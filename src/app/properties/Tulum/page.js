...
function TulumProperties() {
  ...
  useEffect(() => {
    const tulumProperties = allProperties.filter(
      (p) => p.ubicacion === "Tulum"
    );

    setFilteredProperties(tulumProperties);
  }, [allProperties]);
  useEffect(() => {
    setTimeout(() => {
      setIsLoading(false);
            ...
            {isLoading ? (
              ...
              <div className="mx-auto grid gap-2 lg:grid-cols-3 ">
                {filteredProperties.length > 0 ? (
                  ...
                  <h2 className="text-center mt-8 text-xl lg:text-2xl font-black text-red-500">
                  {language === es
                            ? es.wesorry
                            : en.wesorry}
                  </h2>
                  ...
                  >
                    {language === es
                            ? es.seeOtherLocations
                            : en.seeOtherLocations}
